...
function cek_data($nama, $pass)
{
    ...
    $pass = escape($pass);
    // $pass = md5($pass);
    $query = "SELECT * FROM users WHERE username='$nama' AND
password='$pass'";
    global $link;
...
{
    $query = "UPDATE kategori SET nm_kategori='$kategori' WHERE
    id_kategori='$id'";
    return run($query);
}
...
function tambahArtikel(
    ...
    $tempat = str_replace(
        '../',
        'http://localhost/week7/beritaonline/',
        $tempat
    );
    $judul = escape($judul);
    $isi = escape($isi);
...
function updateArtikel(
    ...
    if (!empty($file)) {
        ...
        $tempat = str_replace(
            '../',
            'http://localhost/beritaonline/',
            $tempat
        );
    } else {
        //jika file gambar masih ada, maka value variabel $tempat sama 
        //dengan value dari variabel $url
        $tempat = $url;
    }
    $judul = escape($judul);
    $isi = escape($isi);
