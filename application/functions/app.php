...
{
    global $link;
    if ($result = mysqli_query($link, $query) or die('gagal menampilkan
data')) {
        return $result;
    }
}
...
{
    $query = 
    "SELECT a.*, b.nm_kategori, c.username 
    FROM konten a 
...
{
    $query = 
    "SELECT * FROM konten ORDER BY id_konten DESC";
    return result($query);
}
function TampilkanSemuaArtikelByKategori($id)
{
    $query = "SELECT * FROM konten WHERE id_kategori='$id' ORDER BY
    id_konten DESC";
    return result($query);
}
